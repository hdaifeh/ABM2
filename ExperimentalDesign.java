    ...
    public void lectureEntree(String fileName, int ligne, int frequenceSauvegarde, boolean entete) {
        try {
            ...
            if (entete & !printTrajectory) {
                ...
                System.out.print("dÃ©chetterie" + ";"); // quantity in local composting ??? no in waste collection centre
...
// objective parameters local composting behaviour 11 and 12, sorting for door-to-door collection 10 and 14
    ...
    public void ecritureResultatsComputed(int nbYears) {
        try {
            ...
            else { a=7 ; b =8 ; } // annÃ©e 2024:::{ a=7 ; b =8 ; }:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::2024!!!!!!!
            for (int i = a; i < b; i++) { // annee 2030 est 13, 23 est 2040
                ...
                for (v = 0; v < nbSubterritories; v++) {
                    System.out.print(Double.toString(myDyn.myTerrit[v].Î±cf[i]).replace(".", ",") + ";");
                }
                for (v = 0; v < nbSubterritories; v++) {
                    System.out.print(Double.toString(myDyn.myTerrit[v].Î±cg[i]).replace(".", ",") + ";");
                }
                for (v = 0; v < nbSubterritories; v++) {
                    System.out.print(Double.toString(myDyn.myTerrit[v].Î±sf[i]).replace(".", ",") + ";");
                }
                for (v = 0; v < nbSubterritories; v++) {
                    System.out.print(Double.toString(myDyn.myTerrit[v].Î±sg[i]).replace(".", ",") + ";");
                }
                for (v = 0; v < nbSubterritories; v++) {
            ...
            e.printStackTrace();
            System.out.println();
            System.out.println("j'ai un problï¿½me d'ï¿½criture des rï¿½sultats");
        }
    ...
    int[] solutionForAllForFood; // obligations rÃ©glementaires
    ...
    public void indicatorsObjectives(int nbYears) { // pour Ã©valuer atteinte des objectifs donnÃ©es par gouvernement franÃ§ais
        int bound = nbYears + 1;
        // Volume of hsh waste = Ball - compostage local (rÃ©duction de 15 % par rapport Ã  2018 attendue pour 2030
        ...
        for (int i = 1; i < bound; i++) {
            ...
            for (int j = 0; j < nbSubterritories; j++) {
                ...
                totalProducedFoodWaste = totalProducedFoodWaste + myDyn.myTerrit[j].Bpf[i]; // Bpf est pour une annÃ©e donnÃ©e !!!!!!!!T
                myDyn.myTerrit[j].indicSubTerritories(i); // calcul des indicateurs de l'Ã©tat du sous-territoire j pour l'annÃ©e i
                // Calcul de intention de solution de tri > 0.95
                totalIntentionForFoodWaste[i] = totalIntentionForFoodWaste[i] + myDyn.myTerrit[j].Î±cf[i] + myDyn.myTerrit[j].Î±sf[i];
                totalIntentionForGreenWasteWithoutDechetterie[i] = totalIntentionForGreenWasteWithoutDechetterie[i] + myDyn.myTerrit[j].Î±cg[i] + myDyn.myTerrit[j].Î±sg[i];
            }
            nbSolutionsForAllForFood[i] = totalCapacityForFoodWaste / totalProducedFoodWaste; // pas de problÃ¨me pour Green tant que Dechetterie a capacitÃ© infinie
            ...
            increaseOfMethanisedFoodWaste[i] = (myDyn.myCommonEquip.Bmf[i] - myDyn.myCommonEquip.Umf[i]) - myDyn.increasedObjectiveOfMethanisedFoodWaste; // traite des dÃ©chets alimentaires mÃ©thanisÃ©s, doivent augmenter de 5700
        ...
        if (nbYears > 5) {
            for (int i = 1; i < bound; i++) {
                if (nbSolutionsForAllForFood[6] < 1.0) { // doit Ãªtre vrai en iter 6, fin 2023
                ...
                if (increaseOfMethanisedFoodWaste[7] < 0) { // 8 = 2025 (la CAM doit rÃ©colter 5700 tonnes de plus de dÃ©chets alimentaires pour mÃ©thanisation)
